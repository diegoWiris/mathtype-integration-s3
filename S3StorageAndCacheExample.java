...
public class S3StorageAndCacheExample implements StorageAndCache {
    ...
    private static final String BUCKET_KEY = "@BUCKET_NAME@";
    private static final String ACCESS_KEY = "@ACCESS_KEY@";
    private static final String SECRET_KEY = "@SECRET_KEY@";
    private static final Region REGION_KEY = Region.getRegion(Regions.EU_WEST_1);
    ...
    public void init(Object obj, Properties config, Cache cache, Cache cacheFormula) {
        ...
        if (s3 == null) {
            ...
            s3.setRegion(REGION_KEY);
        }
        bucketName = config.getProperty(BUCKET_KEY);
    }
    ...
    public String codeDigest(String content) {
        ...
        try {
            ...
            s3.putObject(BUCKET_KEY, key, in, meta);
    ...
    public String decodeDigest(String digest) {
        try {
            ...
            S3Object objectData = s3.getObject(BUCKET_KEY, key);
    ...
    public void deleteCache() {
        try {
            ObjectListing object_listing = s3.listObjects(BUCKET_KEY);
            while (true) {
                for (Iterator<?> iterator = object_listing.getObjectSummaries().iterator(); iterator.hasNext();) {
                    S3ObjectSummary summary = (S3ObjectSummary) iterator.next();
                    s3.deleteObject(BUCKET_KEY, summary.getKey());
                }
            ...
            VersionListing version_listing = s3.listVersions(new ListVersionsRequest().withBucketName(BUCKET_KEY));
            while (true) {
                for (Iterator<?> iterator = version_listing.getVersionSummaries().iterator(); iterator.hasNext();) {
                    S3VersionSummary vs = (S3VersionSummary) iterator.next();
                    s3.deleteVersion(

                        BUCKET_KEY, vs.getKey(), vs.getVersionId());
                }
